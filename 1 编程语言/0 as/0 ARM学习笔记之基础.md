...
但当处于**用户模式**下是没有权限实现模式转换的。若想实现模式切换，只能**通过外部中断或是异常处理过程进行切换**。ARM指令集中了软中断指令SWI (SoftWare Interrupt)。**软中断**是对比硬件中断的概念，用软件方式模拟从而**实现从用户模式切换到特权模式并执行特权程序的机制**。软件中断指令用于产生软中断，实现从任意模式变换到管理模式。在这个过程中CPSR保存到管理模式的SPSR中。
